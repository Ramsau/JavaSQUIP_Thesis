...
we tested on an \textit{AMD Ryzen 7 5800X} as representative for Zen~3 and an \textit{AMD Ryzen 7 7700X} as representative for Zen~4.
...
\label{ch:comparison}
% todo compare including transmission errors
...
The authors of the paper also present a version of the covert channel that works without a shared renderer process,
...
\end{figure}
%todo continue here, replace with table
...
We propose a process-based separation of physical CPU cores.
...
The drawback is that this would have to be implemented at the scheduler level,
requiring an operating system-level update.
...
Thus, we can expect JavaSQUIP to remain a threat for the near future.

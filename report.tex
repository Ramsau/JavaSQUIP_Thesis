...
In this chapter, we go into detail on CPU scheduling and how it has been exploited by prior work.
...
With \textit{Simultaneous Multithreading} (SMT), one physical core is split into two logical cores.
...
An attacker that is co-located with a victim has two vectors for a timing attack: \textit{port contention} and \textit{scheduler queue contention}.
...
Thus both \texttt{Date.now()} and \texttt{performance.now()} give a synchronised timestamp with 1 ms accuracy, making them viable options for synchronisation.
...
This would most likely also be the case in any real-world attack.
...
The aim was to build a showcase for JavaSQUIP that, while showing the capabilities of the covert channel,
should reflect as realistic a scenario as possible.
The showcase consists of two web pages: one receiver and one sender.

The sender represents the victim in a real-world scenario.
...
Given this, the attacker can co-locate with the victim and exploit the SQUIP sidechannel to gain information.

...
Section~\ref{ch:conclusion} outlining potential use cases and discussing the implications of the attack.
...
which threw off the average execution time.
...
If the average execution time is above the delay threshold (see Section~\ref{sec:delaythreshold}), a \texttt{1} is registered, else a \texttt{0}.
Figure~\ref{fig:conrentioncoalesced} shows that scheduler queue contention usually affects around $30-50\%$
of the tries in a timeslice.
...
\textit{buffer instructions} and a \textit{delay loop}.
...
that starts an event handler to start a video on the website.
...
\begin{itemize}
    ...
    \item reuse of renderers by Chrome for reuse optimisation
\end{itemize}
...
As of Q3 of 2024, AMD holds a market share of around $33\%$ in CPUs.
Most of these CPUs run under the Zen~2, Zen~3 and Zen~4 architecture,
meaning that large portion of current PCs is susceptible to this attack.
...
Other than the \texttt{Date.now()}~\cite{datenow} and \texttt{SharedArrayBuffer}~\cite{sharedarraybuffer},
...
% todo why browser mitigations are hard
% - timing functions
% - multithreading
